...
//这是一个TCP 服务器应用回调函数。
//该函数通过UIP_APPCALL(tcp_appcall)调用,实现Web Server的功能.
...
void tcp_server_appcall(void)
{
	...
	if (uip_newdata())//收到客户端发过来的数据
	{
		if((tcp_server_sta&(1<<6))==0)//还未收到数据
		{
	    	...
	    	strcpy((char*)tcp_server_databuf,uip_appdata);
				for(i=0;i<uip_len;i++)
				{
					tcp_server_databuf[i] = ((u8*)uip_appdata)[i];
				}				
	...
	}else if(tcp_server_sta&(1<<5))//有数据需要发送
	{
		...
		s->textlen = tcp_server_databuf[0];//设置数据包长度
...
{								  
	...
	s->textlen=0;
	s->textptr="Connect to Successfully!\r\n";
